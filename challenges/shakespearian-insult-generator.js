...
//function to generate one insult
function generateInsult(){
  var randFirstWord = first_word[Math.floor(Math.random() * first_word.length)];
  console.log(randFirstWord);
};
// generateInsult();

//function to generate two insults
function generateInsults(){
  var randFirstWord = first_word[Math.floor(Math.random() * first_word.length)];
  var randTwoWord = second_word[Math.floor(Math.random() * second_word.length)];
  console.log(randFirstWord + ", " + randTwoWord);
};
// generateInsults();

//function to generate n no# of insults
function generateNInsults(){
  var num = prompt("how many insult words would you like?")
  var all_words = ["yeasty", "weedy", "wayward", "warped", "villainous", "venomed", "vain", "unmuzzled", "tottering", "surly", "spongy", "spleeny", "saucy", "ruttish", "roguish", "reeky", "rank", "qualling", "puny", "puking", "pribbling", "paunchy", "mewling", "mangled", "mammering", "lumpish", "loggerheaded", "jarring", "infectious", "impertinent", "gorbellied", "goatish", "gleeking", "frothy", "froward", "fobbing", "fawning", "errant", "droning", "dissembling", "dankish", "currish", "craven", "clouted", "cockered", "churlish", "bootless", "beslubbering", "bawdy", "artless", "weather-bitten", "unchin-snouted", "toad-spotted", "tickle-brained", "tardy-gaited", "swag-bellied", "spur-galled", "sheep-biting", "shard-borne", "rump-fed", "rude-growing", "rough-hewn", "reeling-ripe", "pox-marked", "pottle-deep", "plume-plucked", "onion-eyed", "motley-minded", "milk-livered", "knotty-pated", "ill-nurtured", "ill-breeding", "idle-headed", "hell-hated", "hedge-born", "hasty-witted", "half-faced", "guts-griping", "full-gorged", "fool-born", "folly-fallen", "fly-bitten", "flap-mouthed", "fen-sucked", "fat-kidneyed", "elf-skinned", "earth-vexing", "dread-bolted", "doghearted", "dizzy-eyed", "dismal-dreaming", "crook-pated", "common-kissing", "clay-brained", "clapper-clawed", "boil-brained", "beetle-headed", "beef-witted", "bat-fowling", "base-court"];
  for (var i = 0; i < num; i++) {
    console.log(all_words[i * (Math.floor(Math.random() * num))]);
    }
  };
// generateNInsults();

// function to allow the user to supply a proper name as an input. ("Why Justin, you villainous toad-spotted strumpet!").
function properName(){
  var name = prompt('Provide a name');
  var insult1 = first_word[(Math.floor(Math.random() * name.length))];
  var insult2 = second_word[(Math.floor(Math.random() * name.length))];
  var insult3 = third_word[(Math.floor(Math.random() * name.length))];
  console.log('Why ' + name + ', you ' + insult1 + ' ' + insult2 + ' ' + insult3 + "!");
};
// properName();
